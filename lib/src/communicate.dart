...
class Communicate {
  ...
  Stream<TTSChunk> _stream() async* {
    ...
    final wssUrl = "${Constants.wssUrl}&ConnectionId=$connectId&Sec-MS-GEC=$secMsGec&Sec-MS-GEC-Version=${Constants.secMsGecVersion}";
    
    final headers = DRM.headersWithMuid(Constants.wssHeaders);
    
    // Connect
    // Note: IOWebSocketChannel.connect supports headers.
    final channel = IOWebSocketChannel.connect(Uri.parse(wssUrl), headers: headers);
      ...
      if (json["Metadata"] is List) {
          for (final meta in json["Metadata"]) {
             ...
             if (type == "WordBoundary" || type == "SentenceBoundary") {
                 ...
                 return Metadata(
                     type: type,
                     offset: (innerData["Offset"] as num).toDouble() + (state["offset_compensation"] as num).toDouble(),
                     duration: (innerData["Duration"] as num).toDouble(),
                     text: innerData["text"]["Text"], // Needs unescape? json decode usually handles basic escapes
                 );
             }
          }
      }

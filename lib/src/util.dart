...
class EdgeTTSUtil {
  ...
  static Iterable<String> splitTextByByteLength(String text, int byteLength) sync* {
    if (byteLength <= 0) {
    ...
    while (bytes.length > byteLength) {
        ...
        if (splitAt <= 0) {
             throw Exception("Maximum byte length is too small or invalid text structure.");
        }
